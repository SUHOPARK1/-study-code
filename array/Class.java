 ...
 *    필요한 코드의 양을 줄이자가 바로 3세대 언어, 객체지향언어가 목표하는바이다.
 *    
 * 2. 객체 (=붕어빵들)
 *    객체란 클래스 변수를 뜻한다.
 *    즉, 객체지향 프로그래밍이란, 클래스 변수를 사용해서 그 변수가 생성하는 내부 메세지를 활용해서
 *    우리가 필요한 프로그램을 만드는것을 객체지향 프로그래밍이라고 한다.
 *    클래스라는 것 자체가 한 가지 책임에 따라 거기에 알맞는 정보를 넣는 칸과, 그리고 기능들이 구현되어
 *    있기 때문에 우리가 필요한 클래스 변수, 즉 객체를 만들어서 우리 프로그램에서 사용하게 되는 것이다.
 *    
 * 3. 클래스의 구성요소
 ...
 *    
 * 5. 메소드
 *    메소드란 해당 클래스의 모든 객체들이 공통적으로 가지고 있는 기능을 메소드라고 한다.
 *    필드와는 다르게 모든 메소드들은 똑같은 로직으로 돌아간다.
 *    예를들어, 학생의 국영수 점수를 합쳐서 총점을 계산하는 메소드는
 *    A 학생이건, B 학생이건 똑같다.
 *      
 * 6. 생성자 (Constructor)
 *    생성자란, 해당 클래스 변수가 초기화 될때
 *    호출되는 메소드이다.
 *    우리가 아무런 생성자를 만들어주지 않으면
 *    자바에서 기본적으로 제공해주는 생성자가
 *    해당 클래스의 필드를 초기해화준다.
 *    만약, 해당 필드의 데이터타입이 참조형이면 null으로 초기화하고 기본형 데이터타입이면 0으로 초기화 한다!
 *    
 ...
 *    
 *    생성자는 메소드이지만 특수하게 return 타입이 존재하지 않는 형태이다.
 *    즉
 *    public 클래스이름() {
 *    
 *    }
 *    으로 선언하게 된다!
 *    
 *    다른 메소드의 오버로딩처럼 파라미터에 따라서 호출되는 생성자도 달라진다!
 *    
 *    마지막으로 만약 사용자가 파라미터가 있는 생성자 선언과 구현 한다면
 *    더 이상 파라미터 없는 생성자는 사용할 수 없다.
 *    따라서 파라미터 있는 생성자와 파라미터 없는 생성자 모두 다 필요하다면
 *    둘 다 만들어야 한다!
 ...
 *       그 외에 메소드나 공간은 확보가 되어있는 상태이다! 즉 NullPointerException은
 *       발생하지 않는다.
 *    B. String string = null
 *       이 코드에서는 String 클래스 객체 string이 선언 되었고 heap 영역의 메모리 주소값까지는
 *       부여가 되었지만, 실제 해당하는 heap 영역의 메모리는 아직 초기화가 안되어서 String 클래스에
 *       관한 내용이 하나도 없는 상태이다.  
 *  
 * 9. java.lang.Object
 *    Object 클래스는 자바가 기본적으로 제공해주는 클래스인데
 *    자바가 기본적으로 제공하던 누군가가 만들었던 뭐든간에 자바의 존재하는 모든 클래스는
 ...
 *    또 다시 선언과 정의를 하지 않아도 쓸 수 있게 만들어놓았다.
 ...
 *       String으로 뽑아 내야 한다면 당연히 toString() 메소드를 오버라이드 해야한다!
 *    B. equals()
 ...
 *     캡슐화란, 클래스의 모든 필요 그리고 외부가 꼭 알아야 하는 몇몇 메소드를 제외하고서는
 *     모두 다 private 접근제한자를 적용해시켜서 외부에서는 이 클래스 객체안의 내용을 알 수 없게 만드는
 *     데이터 은닉화 방법이다.
 *     데이터 은닉화를 하는 방법은 외부가 이 클래스 객체의 필드를 직접 접근해서 값을 저장하거나
 *     불러올 수 없고 꼭 메소드를 이용해서 간접적으로 접근하게 만들기 위함이다.
 *     private으러 설정된 필드에 값을 저장할 때에는
