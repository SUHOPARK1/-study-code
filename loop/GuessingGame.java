...
// "DOWN'이라고 출력되고 작으면 "UP"이라고 출력된다.
// 또한 사용자의 최고기록을 보관을 해서
// 만약 메뉴에서 최고기록 보기를 하면
// 해당 최고기록을 볼 수 있게 한다.
// 플레이가 끝날때, 최고 기록이 바뀌는 경우도 반영한다.
    ...
    public static void main(String []args) {
        ...
        while(true) {
            ...
            if(userChoice == 1) {
                ...
                //    우리가 필요한 난수의 데이터 타입에 맞추어
                //    next ~~~ 메소드를 실행해주면 된다
                //    메소드 ()안에 우리가 숫자를 집어 넣으면
                ...
                // 위의 코드는 computheNumber에 0~99 까지의 값이 들어가게 된다.
                // 하지만 우리가 필요한건 1~100이다. 어떻게 해주어야 할까?
                // 메소드의 결과값에 무조건 +1하게 해주면 된다.
                // 그렇다면 무슨값이 나오더라도 1 증가하기 때문에
                // 1~100사이의 숫자가 나오게 된다!
                
                int computerNumber = random.nextInt(MAX)+1 ;

                
                // 2. 컴퓨터 난수와 사용자 입력 숫자가 다른동안 반복
                
                // computerNumber에는 1~100 사이의 숫자가 들어가 있다
